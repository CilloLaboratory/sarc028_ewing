...
dat_comb_meta %>%
  filter(final_cell_annot %in% c("Tumor_cells_c5","Tumor_cells_c4","Tumor_cells_c1")) %>%
  select(cellID,final_cell_annot,KINSEY_TARGETS_OF_EWSR1_FLII_FUSION_UP,HALLMARK_TGF_BETA_SIGNALING,HLA_signature,HALLMARK_INTERFERON_ALPHA_RESPONSE) %>%
  distinct() %>%
  gather(gene_set,values,-final_cell_annot,-cellID) %>% 
  split(.$gene_set) %>%
